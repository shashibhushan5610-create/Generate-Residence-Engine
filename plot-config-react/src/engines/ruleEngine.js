// ruleEngine.js — ES Module version
import { AUTHORITIES_DATA } from '../data/authoritiesData.js';

const authoritiesData = AUTHORITIES_DATA.authorities;

export function getAuthorities() {
  return authoritiesData;
}

export function getLandUses(authorityId) {
  const auth = authoritiesData[authorityId];
  if (!auth) return [];
  return Object.keys(auth.rules);
}

export function calculateRules(authorityId, landUse, plotAreaSqm, roads, options = {}) {
  const {
  ...
  if (auth.type === 'comprehensive' && rules.feasibility) {
    ...
    if (feat) {
      if (plotAreaSqm < feat.minArea)
        report.errors.push(`Plot Area ${plotAreaSqm} fails min ${feat.minArea} requirement.`);
      const maxRoad =
        roads && roads.length > 0 ? Math.max(...roads.map((r) => r.width)) : 0;
      if (maxRoad < feat.minRoad)
        report.errors.push(`Road Width ${maxRoad} fails min ${feat.minRoad} requirement.`);
    }
  }
  ...
  if (rules.telescopicFAR) {
    ...
    maxPurchasableFAR =
      plotAreaSqm < 150
        ? rules.maxPurchasableFAR?.tiny || 2.25
        : rules.maxPurchasableFAR?.regular || 2.5;
  }

  const maxBuiltUpArea = plotAreaSqm * effectiveBaseFAR;
  const maxGroundCoverageArea = plotAreaSqm * ((rules.maxGroundCoverage || 60) / 100);
  ...
  if (rules.parking && Array.isArray(rules.parking)) {
    ...
    ecsRequiredCount = Math.ceil(
      (maxBuiltUpArea / 100) * (rules.parkingRequirementPer100Sqm || 1)
    );
  }
  ...
  return {
    ...
    socialHousing:
      plotAreaSqm >= 4000 &&
      (developmentType === 'group_housing' || developmentType === 'plotted_multi')
        ? {
            status: 'Mandatory',
            EWS: Math.ceil((options.totalUnits || 1) * 0.1),
            LIG: Math.ceil((options.totalUnits || 1) * 0.1),
          }
