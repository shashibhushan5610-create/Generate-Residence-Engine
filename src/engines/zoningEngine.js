// zoningEngine.js — ES Module port of zoning-engine.js v3.0
...
export const UNIT_CONFIGS = {
  '1BHK': { label: '1BHK Studio/Compact', areaRange: [0, 250], distribution: { Living: 0.25, Bedroom: 0.15, Kitchen: 0.08, Bath: 0.10, Circulation: 0.12, Balcony: 0.08, Core: 0.22 }, minSizes: { master: 12, living: 20, kitchen: 4.5, bath: 3, entry: 2.25, balcony: 3 } },
  '2BHK': { label: '2BHK Standard',       areaRange: [250, 400], distribution: { Living: 0.22, Bedrooms: 0.28, Kitchen: 0.10, Bath: 0.12, Circulation: 0.13, Balcony: 0.10, Core: 0.15 }, minSizes: { master: 16, secondary: 10.5, living: 25, dining: 12, kitchen: 7, bath: 3, balcony: 6 } },
  '3BHK': { label: '3BHK Family',         areaRange: [400, 600], distribution: { Living: 0.20, Bedrooms: 0.35, Kitchen: 0.12, Bath: 0.14, Circulation: 0.12, Balcony: 0.12, Core: 0.15 }, minSizes: { master: 20, secondary: 14, living: 30, dining: 16, kitchen: 10, bath: 4.5, office: 7.5, balcony: 10 } },
  '4BHK+': { label: '4BHK+ Luxury Suite', areaRange: [600, Infinity], distribution: { Living: 0.18, Bedrooms: 0.40, Kitchen: 0.12, Bath: 0.16, Circulation: 0.10, Balcony: 0.15, Core: 0.09 }, minSizes: { master: 30, spa_bath: 10.5, living: 40, kitchen: 15, office: 12, balcony: 20 } },
};
...
export const STRATEGY_META = {
  auto:      { name: 'Option 1 — Centralized Hub',         icon: '🎯', concept: 'Core block in center, zones radiating symmetrically for maximum efficiency.', pros: ['Most efficient circulation', 'Equal access to all zones', 'Perfect for apartment stacks'], cons: ['Center placement can block views', 'Less linear flexibility'], scores: { circulation: 5, social: 4, privacy: 4, wayfinding: 5, outdoor: 4 } },
  compact:   { name: 'Option 2 — Corner-Pinned',           icon: '📌', concept: 'Core anchored at one corner to maximize clear floor plate area.', pros: ['Maximizes usable floor area', 'Excellent for linear plots', 'Clear open social zones'], cons: ['Unequal travel distances', 'Longer corridors in deep plots'], scores: { circulation: 3, social: 5, privacy: 4, wayfinding: 4, outdoor: 4 } },
  open_plan: { name: 'Option 3 — Service Segregation',     icon: '🔄', concept: 'Service core at back; social zones at front for clean guest/service separation.', pros: ['Clean functional separation', 'Service zones hidden from view', 'Luxury feel'], cons: ['Requires significant building depth', 'Back access can be awkward'], scores: { circulation: 4, social: 3, privacy: 5, wayfinding: 4, outdoor: 5 } },
  l_shape:   { name: 'Option 4 — Privacy Split',           icon: '📂', concept: 'Living and bedroom wings separated by a central service core.', pros: ['Maximum privacy for sleeping zones', 'Excellent acoustic separation', 'Dual-aspect exposure'], cons: ['Double corridor length', 'Visual division of floor plate'], scores: { circulation: 3, social: 4, privacy: 5, wayfinding: 4, outdoor: 4 } },
  solar:     { name: 'Option 5 — Open-Plan Soft Zoning',   icon: '💨', concept: 'Minimal walls, mobility-driven spaces with an integrated MEP core.', pros: ['Maximum spatial flexibility', 'Sense of spaciousness', 'Modern lifestyle orientation'], cons: ['Acoustic issues', 'Cooking odors in sleeping areas'], scores: { circulation: 4, social: 5, privacy: 2, wayfinding: 3, outdoor: 4 } },
};
...
function createRectAt(center, w, h, angle = 0) {
  ...
  return pts.map(p => ({ x: center.x + p.x * cos - p.y * sin, y: center.y + p.x * sin + p.y * cos }));
}

function createCoreBlock(axes, strategy) {
  const { depthAxis, widthAxis, dMin, dMax, wMin, wMax } = axes;
  const getPoint = (w, d) => ({ x: w * widthAxis.x + d * depthAxis.x, y: w * widthAxis.y + d * depthAxis.y });
  const angle = Math.atan2(depthAxis.y, depthAxis.x) - Math.PI / 2;
  let cw = (wMin + wMax) / 2;
  let cd = (dMin + dMax) / 2;
  if (strategy === 'compact') { cw = wMin + SYSTEMS.staircase.w / 2 + 1.5; cd = dMin + SYSTEMS.staircase.h / 2 + 1.5; }
  else if (strategy === 'open_plan') { cw = (wMin + wMax) / 2; cd = dMax - SYSTEMS.staircase.h / 2 - 1.5; }
...
function computeServiceSegregation(bv, axes, preset, options = {}) {
  ...
  return {
    ...
    systems: core,
    circulation: [
      { from: polygonCentroid(threshold), to: polygonCentroid(socialFinal), label: 'GUEST' },
      { from: polygonCentroid(socialFinal), to: polygonCentroid(service), label: 'SERVICE' },
      { from: polygonCentroid(socialFinal), to: polygonCentroid(private_), label: 'PRIVATE' },
    ],
    labels: [
...
function computePrivacySplit(bv, axes, preset, options = {}) {
  ...
  return {
    ...
    systems: core,
    circulation: [
      { from: polygonCentroid(threshold), to: polygonCentroid(social), label: 'LIVING' },
      { from: polygonCentroid(social), to: polygonCentroid(private_), label: 'SLEEPING' },
    ],
    labels: [
...
function computeOpenPlanSoft(bv, axes, preset, options = {}) {
  ...
  return {
    ...
    systems: core,
    circulation: [
      { from: polygonCentroid(mainSocial), to: polygonCentroid(masterBack), label: 'FLOW' },
      { from: polygonCentroid(masterBack), to: polygonCentroid(serviceStrip), label: 'SERVICE' },
    ],
    labels: [
...
// ─── Public API ───────────────────────────────────────────────────────────────
export function computeZones(buildableVertices, frontEdgeIndex, plotVertices, options = {}) {
  ...
  const strategyMap = { auto: computeCentralizedHub, compact: computeCornerPinned, open_plan: computeServiceSegregation, l_shape: computePrivacySplit, solar: computeOpenPlanSoft };
  const strategyFn = strategyMap[strategy] || computeCentralizedHub;
...
export function determineUnitType(area) {
