...
window.DXFExporter = (function () {
    ...
    /**
     * Creates a minimal valid DXF structure.
     */
...
`;
    ...
    /**
     * Generates a lightweight POLYLINE DXF string.
     */
    ...
    /**
     * Generates a TEXT entity in DXF.
     */
    ...
    /**
     * Generates a professional ALIGNED DIMENSION (Line + Text + Ticks)
     */
    function generateAlignedDimension(p1, p2, text, offset = 1.0, layer = "A-DIMS") {
        ...
        // Ticks (Drafting style diagonal ticks)
    ...
    /**
     * Main function to generate a full DXF file content from the dashboard data.
     */
    function exportToDXF(geometryData, complianceData, roadsData) {
        let entities = "";

        // 1. Add Plot Boundary
        entities += generateLWPolyline(geometryData.vertices, "PLOT_BOUNDARY", geometryData.isClosed);

        // 2. Add Road Widening Strips
        ...
        // 3. Add Buildable Area
        if (geometryData.isClosed && !geometryData.isSelfIntersecting && window.GeometryEngine.calculateInsetPolygon) {
            ...
            const buildableV = window.GeometryEngine.calculateInsetPolygon(geometryData.vertices, offsets);
            if (buildableV.length > 2) {
                entities += generateLWPolyline(buildableV, "BUILDING_LINE", true);
            }
        }

        // 4. Add Smart Dimensions
        if (window.SmartDimensionEngine) {
            const frontIdx = roadsData.length > 0 ? roadsData[0].sideIndex : 0;

            // Outer Plot Dimensions (Smart Suppression)
            const plotDims = window.SmartDimensionEngine.getPlotDimensions(geometryData.vertices, frontIdx);
            plotDims.forEach(d => {
                entities += generateAlignedDimension(d.p1, d.p2, d.label, 1.25, "A-DIMS");
            });

            // Setback / Widening Dimensions
            if (geometryData.isClosed && !geometryData.isSelfIntersecting && complianceData) {
                const compDims = window.SmartDimensionEngine.getComplianceDimensions(
